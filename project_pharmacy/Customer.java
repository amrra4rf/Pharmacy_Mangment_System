...
public class Customer extends Person {
    ...
    void display_self() {
        ...
        l.appendText("-----------------------------------------------\n");
        l.appendText("Name:" + this.get_name()+"\n");
        l.appendText ("Age:" + this.get_age()+"\n");
        l.appendText  ("Balance: " + this.Balance + "$\n");
        if (gender == 'm' || gender == 'M')
        { l.appendText  ("Gender: Male\n");
         l.appendText("-----------------------------------------------\n");}
        else if (gender == 'f' || gender == 'F'){
           l.appendText  ("Gender: Female\n");
            l.appendText("-----------------------------------------------\n");}
        else
        ...
        for (int index = 0; index < Pharmacy.item_count_in_pharmacy; index++) {
            if (p.items[index].get_name().equals(name))// item found
            {
                if (p.items[index].get_count() >= amount_needed) {
                    ...
                    throw new itemBuyError("Insuffcient Amount Of This Item, There are only");
